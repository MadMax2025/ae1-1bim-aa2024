...
class localesComida(Base):
    __tablename__ = 'locales_comida' # El nombre de la entidad en sqlite
    ...
    def __str__(self):
        return "%s %s %s %s" % (self.nombreLocal, self.direccion, self.localidad,
        self.capacidad)
        
class centrosDeportivos(Base):
    __tablename__ = 'centros_deportivos' # El nombre de la entidad en sqlite
    # Se definen los atributos
    id = Column(Integer, primary_key=True) # este atributo es entero y
                                        # se lo considera como llave
                                        # primaria
    ...
    def __str__(self):
        return "%s %s %s %s" % (self.nombreCentro, self.tamaño, self.localidad,
        self.capacidad)
